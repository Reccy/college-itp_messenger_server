...
function broadcastNextChannel(client_uuid) {
    ...
    for (i = 0; i < channelList.length; i++) {
        if (channelList[i].uuid === client_uuid) {
            console.log(" > [" + globalChannel + "] USER " + client_uuid + " ALREADY CONNECTED TO " + channelList[i].chan);
...
pubnub.subscribe({
    ...
    connect: function(m) {
        console.log(" > [" + globalChannel + "] CHANNEL CONNECTED");
    },
...
/* Check channel list every 60 seconds */
setInterval(function(){verifyChannelList();}, 30*1000);
